...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |Taller Funcional Jasson Rodríguez|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;Función factorial
;Parámetros:
...
(define (eliminar-arbol elemento arbol)(
                                       cond
                                        ((equal? elemento (valor arbol)) (
                                                                          cond
                                                                           ...
                                                                           (else (list (mayor (izq arbol)) (eliminar-arbol (mayor (izq arbol)) (izq arbol)) (der arbol)))))                              
                                        ((< elemento (valor arbol)) (list (valor arbol) (eliminar-arbol elemento (izq arbol)) (der arbol)))
                                        ((> elemento (valor arbol)) (list (valor arbol) (izq arbol) (eliminar-arbol elemento (der arbol))))
                                        ))
...
;Ejemplos de ejecucion de las funciones
(define ejemplos (begin
                   (write (factorial 5))
                   (newline)
                   (write (fib 10))
                   (newline)
                   (write (miembro 'c '(a b c d)))
                   (newline)
                   (write (eliminar 'b '(a b c d)))
                   (newline)
                   (write (qsrt '(8 3 5 2 4 6)))
                   (newline)
                   (write (automovil '(Hatchback Suzuki Forza1 Rojo si Manual) '(Tipo Marca Modelo Color AC Transmisión)))
                   (newline)
                   (write (eliminar-arbol 4 '(7 (3 1 4) (15 12 19))))
                   (newline)
                   (write (bfs 'a 'f '(
                                       (a (b c))
                                       (b (d e))
